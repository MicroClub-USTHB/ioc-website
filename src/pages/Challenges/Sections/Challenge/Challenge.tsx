...
const Challenge: React.FC<RouteComponentProps> = (props) => {
    ...
    let [dayNumber, type]: [string | number, 'main' | 'side'] = location.pathname.split('/').slice(-2) as [string, 'main' | 'side'];
    dayNumber = Number(dayNumber[dayNumber.length - 1]);
    const { data: days } = useGetDaysQuery(null);
    const language = useSelector<RootState>((state) =>(state.common.language=== "english"?"EN":"FR")) as "EN" | "FR";
    const { data: day, isLoading: dayLoading } : { data?: ExtendedDay, isLoading: boolean }= useGetDayQuery({ _id: days ? days[dayNumber - 1]._id : "" });
